...
public:
    ...
    void Execute(std::vector<std::vector<int>>& grid, int tileId, const SelectTool& selectTool, std::stack<std::shared_ptr<BuilderCommand>>& undoStack, std::stack<std::shared_ptr<BuilderCommand>>& redoStack);
private:
